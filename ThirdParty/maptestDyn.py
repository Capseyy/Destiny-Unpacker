...
for file in os.listdir(path)[::-1]:
    if fnmatch.fnmatch(file,'w64_terra*'):       #Customize this to what pkgs you need 2. Can wildcard with * for all packages, or all of a certain type.
...
class LoadZone:
    ...
    def OutputCFG(self):
        ...
        for Static in self.StaticMeta:
            ...
            file=open("C:/Users/sjcap/Desktop/MyUnpacker/DestinyUnpackerNew/new/MapExtracter/data/Statics/Instances/"+self.Statics[Index]+".inst","a")
...
def InitialiseFiles(loadfile,InstCount,Ref):
    lengths=[]
    for File in os.listdir(custom_direc):
        if File != "Statics":
            if File.lower() == loadfile:
                Load=LoadZone(File,InstCount,Ref)
...
def TryNewFile():
    ...
    for SubLoad in SubLoads:
        ...
        for File in os.listdir(custom_direc):
            if File != "Statics":
                if File.lower() == Name:  #change back to LHash  80f847c5
                    Tables=[]
                    file=open(custom_direc+"/"+File,"rb")
                    print(File)
                    file.seek(0x50)
    ...
    InitialiseFiles(LoadFiles[ans2-1],InstCount[ans2-1],refs[ans2-1])
