...
class LoadZone:
    def __init__(self,Name,InstCount):
        self.FileName=Name
        self.Statics=[]
        self.StaticMeta=[]
        self.PullStatics()
    ...
    def RipStatics(self):
        for Static in self.Statics:
            cmd="d2staticextractor.exe -p "+path+" -o "+custom_direc+"/Statics/Statics -l -i "+Static
            #print(cmd)
...
def InitialiseFiles(loadfile,InstCount):
    lengths=[]
    for File in os.listdir(custom_direc):
        if File != "Statics":
            if File.lower() == loadfile:
                Load=LoadZone(File,InstCount)
...
def TryNewFile():
    ...
    InitialiseFiles(LoadFiles[ans2-1],InstCount[ans2-1])
