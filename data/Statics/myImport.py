...
def assemble_map():
    ...
    #py.data.collections.new(str(Name))
    #py.context.scene.collection.children.link(bpy.data.collections[str(Name)])
    #py.context.view_layer.active_layer_collection = bpy.context.view_layer.layer_collection.children[str(Name)]

    #py.ops.import_scene.fbx(filepath=FileName, use_custom_normals=True, ignore_leaf_bones=True, automatic_bone_orientation=True) #Just imports the fbx, no special settings needed
    
    #assign_materials()
    add_to_collection() 

    #ewobjects = bpy.data.collections[str(Name)].objects
    #bjects = bpy.data.objects
    #or O in objects:
    #   print(str(O.name)[:8])
    #print(newobjects)
    ...
    if Is_Map():
        ...
        #ewobjects = bpy.data.collections[str(Name)].objects #Readds the objects in the collection to the list
